...
public class ZegoApiManager {
     ...
     * 外部滤镜开关.
     */
    private boolean mUseVideoFilter = true;
     ...
     * 高级功能.
     */
    private void openAdvancedFunctions(Resources res){
        ...
        if(mUseVideoFilter){
            // 外部滤镜
            VideoFilterFactoryDemo videoFilterFactoryDemo = new VideoFilterFactoryDemo(res);
    ...
    private void init(Resources res,long appID, byte[] signKey){

        initUserInfo();

        // 开发者根据需求定制
        openAdvancedFunctions(res);
     ...
     * 初始化sdk.
     */
    public void initSDK(Resources res){
        ...
        init(res,mAppID, mSignKey);
    }

    public void reInitSDK(Resources res,long appID, byte[] signKey) {
        init(res,appID, signKey);
    }
